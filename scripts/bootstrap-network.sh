...
while ((SECONDS < deadline)); do
  [[ "$(docker inspect --format '{{.State.Health.Status}}' "${NETWORK_NAME}-explorer" 2>/dev/null || true)" == "healthy" ]] && break
  sleep 2
done
[[ "$(docker inspect --format '{{.State.Health.Status}}' "${NETWORK_NAME}-explorer" 2>/dev/null || true)" == "healthy" ]] || { echo "explorer did not become healthy." >&2; exit 1; }
